...
Possíveis dúvidas: existe separado do construtor padrão porque accept() já devolve um handle pronto — não faz sentido criar outro socket do zero e depois trocar
========================*/
...
Possíveis dúvidas: RAII = o recurso (socket) é liberado automaticamente pelo destrutor, então não precisamos lembrar de fechar manualmente em todo lugar
========================*/
Socket::~Socket()
{
    close();
...
Como faz: Copia o handle do objeto "other" e zera o handle dele (INVALID_SOCKET), pra o destrutor do "other" não fechar o socket que acabou de ser transferido
Possíveis dúvidas: por que não copiar em vez de mover? Dois objetos com o mesmo handle tentariam fechar o mesmo socket no destrutor, causando erro (double close)
========================*/
...
Como faz: Monta um sockaddr_in com o IP (convertido de string pra binário via inet_pton) e a porta (convertida pra network byte order via htons), depois chama connect()
Possíveis dúvidas: htons converte a porta do formato do seu computador (little-endian, geralmente) pro formato de rede (big-endian) — sem isso a porta chegaria errada do outro lado
========================*/
void Socket::connectTo(const std::string& host, unsigned short port)
{
    ...
    if(::connect(handle_, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) == SOCKET_ERROR)
    {
...
Como faz: Preenche um sockaddr_in com INADDR_ANY (aceita conexões vindas de qualquer interface de rede da máquina) e a porta desejada, depois chama bind()
Possíveis dúvidas: só faz sentido no lado servidor — o cliente não precisa, o sistema operacional escolhe uma porta livre sozinho ao conectar
========================*/
...
Possíveis dúvidas: backlog é quantas conexões podem esperar na fila antes de serem processadas por accept(); SOMAXCONN deixa o SO escolher um valor razoável
========================*/
...
Possíveis dúvidas: o socket original (o que fez bind/listen) continua livre pra aceitar outras conexões — o handle retornado aqui é um socket separado, dedicado a esse cliente específico
========================*/
...
Possíveis dúvidas: por que não mandar tudo numa chamada só? send() pode enviar só uma parte dos dados de uma vez (short write), principalmente em mensagens grandes — por isso o loop
========================*/
...
Possíveis dúvidas: TCP é um fluxo contínuo de bytes, sem separação nativa de mensagens — por isso o protocolo do projeto usa '\n' como delimitador, pra saber onde uma mensagem termina
========================*/
...
Possíveis dúvidas: por que checar handle_ != INVALID_SOCKET antes? Porque close() pode ser chamado mais de uma vez (ex: manualmente e depois pelo destrutor), e fechar um handle já fechado é erro
========================*/
void Socket::close()
{
    if (handle_ != INVALID_SOCKET) 
...
Possíveis dúvidas: útil pra checar o estado depois de um move (o objeto "esvaziado" fica com handle_ == INVALID_SOCKET, logo isValid() retorna false)
========================*/
